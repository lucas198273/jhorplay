      ...
      <div className="container mx-auto px-4">
        ...
        <div className="hidden lg:grid grid-cols-4 gap-6">
          {plans.map((plan, index) => (
            <div key={index} className="bg-blue-800 p-6 rounded-lg text-white text-center shadow-lg">
              ...
              <p className="mt-4 text-xl font-bold">R$ {plan.price} <span className="text-gray-300">{plan.installments}</span></p>
              {plan.trial && <button className="mt-4 bg-white text-blue-800 hover:bg-gray-200 py-2 px-4 rounded">TESTE GRÁTIS</button>}
            </div>
          ))}
        </div>
        ...
        <div className="lg:hidden overflow-x-auto">
          <div className="flex space-x-4 px-4">
            {plans.map((plan, index) => (
              <div key={index} className="bg-blue-800 p-6 rounded-lg text-white text-center shadow-lg min-w-[80%] sm:min-w-[60%]">
                <h3 className="text-2xl font-bold">{plan.name}</h3>
                <p className="text-lg text-gray-300">{plan.duration}</p>
                <ul className="mt-4 text-sm text-gray-200 space-y-1">
                  <li>Pacote Completo</li>
