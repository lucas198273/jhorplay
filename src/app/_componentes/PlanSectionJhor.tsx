      ...
      <div className="relative z-10 container max-w-7xl mx-auto px-4 py-10 bg-[rgba(0,0,0,0.6)] rounded-lg">
        ...
        <div className="hidden lg:grid grid-cols-4 gap-8 justify-items-center">
          {plans.map((plan, index) => (
            <div
              key={index}
              className="bg-gradient-to-r from-black to-[#0f172a] p-6 rounded-lg text-white text-center shadow-lg"
              data-aos="fade-up"
        ...
        <div className="lg:hidden overflow-x-auto -mx-4 px-4">
          <div className="flex space-x-4 w-max">
            {plans.map((plan, index) => (
              <div
                key={index}
                className="min-w-[280px] bg-gradient-to-r from-black to-[#0f172a] p-4 rounded-xl text-white text-center shadow-md"
                data-aos="fade-up"
                data-aos-delay={index * 100}
              >
                <h3 className="text-2xl font-bold">{plan.name}</h3>
